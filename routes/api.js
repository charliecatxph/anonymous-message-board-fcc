      ...
      Message.find({ board: board }, (err, data) => {
        if (err) {
          ...
          const a = data.map(data => ({
            ...
            replies: data.replies.sort((d1, d2) => d2.created_on - d1.created_on).slice(0, 3).map(d => ({
              _id : d._id,
              text : d.text,
          ...
          res.json(a.sort((d1, d2) => d2.bumped_on - d1.bumped_on).slice(0, 10))
        }
      })
    })
    .post((req, res) => {
          ...
          if (err) {
            ...
            if (!data || data.length === 0) {
              ...
              res.json(data.map(d => ({
                ...
                created_on : d.created_on,
                bumped_on : d.bumped_on,
                replies : d.replies.sort((d1, d2) => d2.created_on - d1.created_on).map(d => ({
                  _id : d._id,
                  text : d.text,
                  created_on : d.created_on
                })).slice(0,3),
          ...
          if (err) {
            ...
            if (!data) {
              ...
              try {
                ...
                if (bcrypt.compareSync(delete_password, findSubdocument.delete_password)) {
                  data.bumped_on = new Date();
